...
Because JavaScript was originally designed to run only in browsers. Node.js lets JavaScript run on the server side too — outside the browser.

Ravi Tambade (10:54) Excellent! So Node.js is a server-side runtime for JavaScript — like how .NET runs C# or Java runs in JVM.
...
&nbsp;Every technology stack needs a runtime. If you build in Java → you need JVM. If you build in .NET → you need CLR. If you build with Node → you have the Node runtime.



In the Node runtime, we use a framework called Express, and within Express, we can use a view engine like EJS. It allows us to mix HTML + JavaScript together — but this JavaScript runs on the server side, not the browser.
...
Every browser — Chrome, Edge, Firefox, Safari — is like a small runtime environment too. It can send requests, receive responses, and render what you see on the screen.
...
Node.js does not run inside the browser. It runs on the developer machine or server — it gives developers a runtime to build, test, and bundle their front-end code.
...
✅ Source Code — written in Java, C#, or TypeScript ✅ SDK (Software Development Kit) — includes compilers, libraries, and tools ✅ IDE — where we write, debug, and run code (like VS Code, IntelliJ, or Visual Studio) ✅ Server — to test or host our applications (Kestrel, Tomcat, Express)
...
Ravi Tambade (43:10) Exactly! Developers need both — an environment to code and an environment to run.
...
If the user changes data in the UI → the model updates.

If the model changes → the UI updates automatically.
...
&nbsp;Angular CLI gives you an entire toolbox of commands:
...
When you create a project using Angular CLI, it doesn’t just build code — it also prepares you for testing.
...
A compiler takes your source code and converts it into executable code — like .exe for Windows or .dll for .NET. A transpiler, on the other hand, converts code from one language into another language — not directly into an executable.



In Angular, we write our code in TypeScript (.ts files). But browsers don’t understand TypeScript — they only understand JavaScript.
